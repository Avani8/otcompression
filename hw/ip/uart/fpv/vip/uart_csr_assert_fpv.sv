...
// Block: uart
module uart_csr_assert_fpv import tlul_pkg::*; (
  input clk_i,
  input rst_ni,
...
// for all the regsters, declare assertion
  ...
  `ASSERT(intr_state_wr_A, wr_P(7, 6'h0, i_uart.reg2hw.intr_state.q, 0), clk_i, !rst_ni)
  `ASSERT(intr_state_rd_A, rd_P(7, 6'h0, i_uart.hw2reg.intr_state.d), clk_i, !rst_ni)

  // read/write assertions for register: intr_enable
  `ASSERT(intr_enable_wr_A, wr_P(7, 6'h4, i_uart.reg2hw.intr_enable.q, 0), clk_i, !rst_ni)
  `ASSERT(intr_enable_rd_A, rd_P(7, 6'h4, i_uart.reg2hw.intr_enable.q), clk_i, !rst_ni)

  // read/write assertions for register: intr_test
  `ASSERT(intr_test_wr_A, wr_ext_P(7, 6'h8, i_uart.reg2hw.intr_test.q, 0), clk_i, !rst_ni)

  // read/write assertions for register: ctrl
  `ASSERT(ctrl_wr_A, wr_P(31, 6'hc, i_uart.reg2hw.ctrl.q, 0), clk_i, !rst_ni)
  `ASSERT(ctrl_rd_A, rd_P(31, 6'hc, i_uart.reg2hw.ctrl.q), clk_i, !rst_ni)

  // read/write assertions for register: status
  `ASSERT(status_rd_A, rd_ext_P(5, 6'h10, i_uart.hw2reg.status.d), clk_i, !rst_ni)

  // read/write assertions for register: rdata
  `ASSERT(rdata_rd_A, rd_ext_P(7, 6'h14, i_uart.hw2reg.rdata.d), clk_i, !rst_ni)

  // read/write assertions for register: wdata
  `ASSERT(wdata_wr_A, wr_P(7, 6'h18, i_uart.reg2hw.wdata.q, 0), clk_i, !rst_ni)

  // read/write assertions for register: fifo_ctrl
  `ASSERT(fifo_ctrl_wr_A, wr_P(6, 6'h1c, i_uart.reg2hw.fifo_ctrl.q, 0), clk_i, !rst_ni)
  `ASSERT(fifo_ctrl_rd_A, rd_P(6, 6'h1c, i_uart.hw2reg.fifo_ctrl.d), clk_i, !rst_ni)

  // read/write assertions for register: fifo_status
  `ASSERT(fifo_status_rd_A, rd_ext_P(21, 6'h20, i_uart.hw2reg.fifo_status.d), clk_i, !rst_ni)

  // read/write assertions for register: ovrd
  `ASSERT(ovrd_wr_A, wr_P(1, 6'h24, i_uart.reg2hw.ovrd.q, 0), clk_i, !rst_ni)
  `ASSERT(ovrd_rd_A, rd_P(1, 6'h24, i_uart.reg2hw.ovrd.q), clk_i, !rst_ni)

  // read/write assertions for register: val
  `ASSERT(val_rd_A, rd_ext_P(15, 6'h28, i_uart.hw2reg.val.d), clk_i, !rst_ni)

  // read/write assertions for register: timeout_ctrl
  `ASSERT(timeout_ctrl_wr_A, wr_P(31, 6'h2c, i_uart.reg2hw.timeout_ctrl.q, 0), clk_i, !rst_ni)
  `ASSERT(timeout_ctrl_rd_A, rd_P(31, 6'h2c, i_uart.reg2hw.timeout_ctrl.q), clk_i, !rst_ni)
