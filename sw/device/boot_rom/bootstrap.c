...
static int check_frame_hash(const frame_t *f) {
  ...
  int b =2;
 //sum(a,b);
 uart_send_str("Processing sum: ");
 uart_send_uint(sum(a,b), 32);
   char*  in = "1223233453";
   char*  out = "0000000000";
  // uint8_t *uncompressed_text;
   ...
   //uint32_t  compressed_size;
    ...
    //uncompressed_text [uncompressed_size]="0" ;
   //compressed_text=6000;
   // compressed_text = malloc(malloc_size);
  //  for (uint32_t i=0; i< 10; i++){
   uint8_t *uncompressed_text= (uint8_t *)in; 
   uint8_t *compressed_text= (uint8_t *)out; 
   //}
   // compressed_size = lz77_compress(uncompressed_text, uncompressed_size, compressed_text, 1);
  lz77_compress(uncompressed_text, uncompressed_size, compressed_text, 1);
  uart_send_str("Processing frame no: ");
  for (uint32_t i=0; i< 10; i++){
     // uart_send_uint(uncompressed_text[i],8);
      uart_send_uint(compressed_text[i],8);}
